...
public class OpFrogEntity extends FrogEntity {

	public OpFrogEntity(EntityType<? extends AnimalEntity> entityType, World world) {
	...
	public void tick() {
		...
		if (ticks > 20) {
			...
			if (world.getClosestPlayer(this, 3) != null) {

				PlayerEntity closest = world.getClosestPlayer(this, 3);
				Entity entity;
				world.playSoundFromEntity(null, this, SoundEvents.ENTITY_FROG_TONGUE, SoundCategory.NEUTRAL, 2.0f,
						1.0f);

				this.setPose(EntityPose.USING_TONGUE);

				this.setFrogTarget(closest);
				Optional<Entity> optional = this.getFrogTarget();
				if (optional.isPresent() && (entity = optional.get()).isAlive()) {
					this.tryAttack(entity);

					this.setPose(EntityPose.USING_TONGUE);
					if (this.getPose() == EntityPose.USING_TONGUE) {
						this.lookAtEntity(closest, 180.0f, 180.0f);
						if (!entity.isAlive()) {
	...
	public AttributeContainer getAttributes() {
		if (attributeContainer == null)
			attributeContainer = new AttributeContainer(MobEntity.createMobAttributes()
					...
					.add(EntityAttributes.GENERIC_MAX_HEALTH, 30.0).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 5.0)
					.build());
	...
	class FrogLookControl
		...
		protected boolean shouldStayHorizontal() {
			return OpFrogEntity.this.getFrogTarget().isEmpty();
		}
	}
